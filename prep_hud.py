...
# ⚠️ 這兩張表也是「認不認識這支工作」的白名單:_parse_args 遇到不在 RUN_LABEL
# 裡的 --job 會**退回 prep**（不要因為打錯字就開不起來）。所以新增一支會動
# Chrome 的工作時，這裡一定要跟著加 —— 漏了的話浮窗會去看收文那支的進度，
# 顯示的是上一輪的舊訊息，還會以為早就跑完了。
RUN_LABEL = {"prep": "收新公文中…請不要動滑鼠",
             "send": "送陳核中…請不要動滑鼠",
             "archive": "存查歸檔中…請不要動滑鼠",
             # 張貼不碰 edoc、不用插卡，但它自己開的那個 Chrome 正在填校網的
             # 表單 —— 那段一樣不能動滑鼠。
             "web": "貼上校網中…請不要動滑鼠"}
