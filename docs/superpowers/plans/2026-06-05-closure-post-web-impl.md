...
- 登入頁 `https://www.sssh.tp.edu.tw/passport/tpeEntrance`：`#login-user-name`、`#login-password`、`button.btn-primary[type=submit]`（**JS click**）。
- 公告板 `https://www.sssh.tp.edu.tw/nss/s/main/p/NetworkCenterAnnoucement`：「新增公告」鈕 → CKEditor 5 表單。
...
```python
LOGIN_URL = "https://www.sssh.tp.edu.tw/passport/tpeEntrance"
...
def _submit_announcement(driver, title, body):
    ...
    try:
        ...
        """)
        if not clicked:
            _stop_banner("找不到「新增公告」按鈕(此帳號在此板可能無權限)")
            return False
        time.sleep(2.5)
